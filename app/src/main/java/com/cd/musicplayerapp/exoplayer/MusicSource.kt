...
interface MusicSource : Iterable<MediaMetadataCompat> {
    ...
    val shuffledSongs: MutableList<MediaMetadataCompat>
...
abstract class AbstractMusicSource: MusicSource {
    ...
    override val shuffledSongs: MutableList<MediaMetadataCompat>
        get() {
            val newSongs = ArrayList(catalog)
            newSongs.shuffle()
            return newSongs
        }
...
fun <T : List<MediaMetadataCompat>> T.asMediaSource(dataSourceFactory: DefaultDataSource.Factory): ConcatenatingMediaSource {
...
fun <T : List<MediaMetadataCompat>> T.asMediaItems(): List<MediaBrowserCompat.MediaItem> {
